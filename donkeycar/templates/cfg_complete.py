...
DEFAULT_MODEL_TYPE = 'linear'   #(linear|tflite_linear|coral_tflite_linear|categorical|rnn|imu|behavior|3d|localizer|latent)
...
##When racing, to give the ai a boost, configure these values. #元設定
...
AI_LAUNCH_DURATION = 0.5            # the ai will output throttle for this many seconds
AI_LAUNCH_THROTTLE = 0.3            # the ai will output this throttle value
AI_LAUNCH_ENABLE_BUTTON = 'R2'      # this keypress will enable this boost. It must be enabled before each use to prevent accidental trigger.
