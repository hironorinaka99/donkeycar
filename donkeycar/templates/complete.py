...
def drive(cfg, model_path=None, use_joystick=False, model_type=None, camera_type='single', meta=[] ):
    ...
    class DriveMode:
        def run(self, mode, 
            ...
            dis_L_LKA_range = 50.0 #左センサーLKA動作範囲
            dis_R_LKA_range = 50.0 #右センサーLKA動作範囲
            ...
            if mode == 'user': 
                ...
                if distanceLL > 140 and distanceL > 100 and distanceC > 120 and distanceR > 50 and distanceRR > 50 and abs(pilot_angle) < 0.4: #順全開条件

                    #print("準全開条件成立")
                    #Sentan
                    #if distanceL > 120 and distanceC > 150 and distanceR > 60 and abs(pilot_angle) < 0.3: #全開条件
                    if distanceL > 120 and distanceC > 150 and distanceR > 100 and abs(pilot_angle) < 0.3: #全開条件
                        #print("全開条件成立")
                        if dis_gapL >= 0 and dis_gapC >= 0 and dis_gapR >=0: #前のセンサー距離がどれも縮まっていない
                            #print("ギャップ条件成立")
                ...
                #中距離で距離センサーのギャップ（縮まり方）が大きいときは大減速
                elif (distanceL < 60 and distanceL > 20 and dis_gapL < -2.0 and pilot_angle < -0.3) or (distanceC < 100 and distanceC > 25 and dis_gapC < -8.0 and abs(pilot_angle) < 0.5) or (distanceR < 60 and distanceR > 20 and dis_gapR < -3.0 and pilot_angle > 0.3): #前センサーで障害物（距離センサーが縮まっている）発見
                    print("front left gap %3.1f cm" % dis_gapL + "front cencer gap %3.1f cm" % dis_gapC + "front right gap %3.1f cm" % dis_gapR)
                    user_throttle *= 0.0
                    print("急速接近中のため　スロットル０")

                #遠くで距離センサーのギャップ（縮まり方）が大きいときは中減速
                elif (distanceL < 80 and distanceL > 60 and dis_gapL < -2.0 and pilot_angle < -0.3) or (distanceC < 140 and distanceC > 100 and dis_gapC < -8.0 and abs(pilot_angle) < 0.5) or (distanceR < 80 and distanceR > 60 and dis_gapR < -3.0 and pilot_angle > 0.3): #前センサーで障害物（距離センサーが縮まっている）発見
                    print("front left gap %3.1f cm" % dis_gapL + "front cencer gap %3.1f cm" % dis_gapC + "front right gap %3.1f cm" % dis_gapR)
                    user_throttle *= 0.0
                    print("急速接近中のため　スロットル０．０")

                #距離センサーのギャップ（縮まり方）がそこまで大きくないときは小減速
                elif (distanceL < 80 and distanceL > 20 and dis_gapL < -1.1 and pilot_angle < -0.3) or (distanceC < 150 and distanceC > 25 and dis_gapC < -5.0 and abs(pilot_angle) < 0.5) or (distanceR < 80 and distanceR > 20 and dis_gapR < -2.1 and pilot_angle > 0.3): #前センサーで障害物（距離センサーが縮まっている）発見
                    print("front left gap %3.1f cm" % dis_gapL + "front cencer gap %3.1f cm" % dis_gapC + "front right gap %3.1f cm" % dis_gapR)
