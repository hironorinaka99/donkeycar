            ...
            if mode == 'user': 
                ...
                if distanceLL < dis_LL_rev_range or distanceL < dis_L_range or distanceC < dis_C_range or distanceR < dis_R_range or distanceRR < dis_RR_rev_range :
                    ...
                    if time_dis_gap > dis_timer_all: #初期タイマー無反応（下記数値より大きいこと）
                        ...
                        if max(distanceL, distanceC, distanceR) == distanceC and distanceL < dis_L_range *1.5 and distanceC < dis_C_range *1.5 and distanceR < dis_R_range *1.5 and distanceLL > dis_LL_rev_range and distanceRR > dis_RR_rev_range: #左右前センサより中央が遠く、前のセンサー3つがどれも近く(係数倍）、横センサーが反応していない条件（角に向いてしまった場合）
